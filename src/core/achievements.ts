 ...
 *
 * 1. An achievement is checked once and kept forever. Nothing in this file
 *    re-evaluates a locked achievement against old stats — a season that was
...
/** Where an achievement sits in the case, which is also how it's grouped. */
export type AchievementTier = 'moment' | 'season' | 'career' | 'honor';
...
/** One earned achievement, stamped with when and where. */
...
/** The game just finished, as the achievement tests want to read it. */
...
const g = (ctx: AchievementContext): GameCheck | undefined => ctx.game;
...
export const TIER_ORDER: AchievementTier[] = ['moment', 'season', 'career', 'honor'];
 ...
 *
 * Safe to call as often as you like: an achievement already in the list is
...
export function checkAchievements(
  ...
  for (const achievement of ACHIEVEMENTS) {
    ...
    if (!achievement.test(ctx)) continue;
    unlocked.push({
      id: achievement.id,
...
export function achievementProgress(unlocked: readonly UnlockedAchievement[]): {
  earned: number;
  total: number;
} {
