...
export class PlaySim {
  ...
  private resolveForceAt(base: BaseId, receiver: FielderState, onFoot = false): void {
    ...
    let victim = this.forcedRunnerAt(base);
    let tagged = false;
    if (!victim) {
    ...
    if (victim) {
      ...
      this.lastEvent = onFoot
        ? receiver.isUser
          ? tagged
            ? 'You tag him at the bag!'
            : 'You step on the bag — got him!'
          : `${receiver.id} ${tagged ? 'tags him at the bag' : 'steps on the bag'}. Out!`
        : tagged
          ? credited
            ? 'Tagged him!'
            : 'Tagged out!'
  ...
  /**
   * The runner a fielder holding the ball on `base` would tag: the one
   * scrambling back to it who is closest, or nobody. `within` limits it to
   * runners that near the bag, as a fraction of a basepath — a fielder
   * stepping on the bag tags a man arriving, not one eighty feet away.
   */
  private retreatingRunnerAt(base: BaseId, within = Infinity): RunnerState | null {
    let victim: RunnerState | null = null;
    for (const runner of this.runners) {
      if (runner.out || runner.at >= 4) continue;
      if (!this.isRetreating(runner) || runner.at % 4 !== base) continue;
      if (runner.progress > within) continue;
