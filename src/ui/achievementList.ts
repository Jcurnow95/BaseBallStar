/**
 * How an achievement looks, in the two places it shows up: the panel that
 ...
 */
import type { Achievement, UnlockedAchievement } from '../core/achievements';
import { LEVELS } from '../core/league';
import { esc } from './dom';

/** One row in the case, earned or not. */
export function achievementRowHtml(
  achievement: Achievement,
  earned: UnlockedAchievement | undefined,
): string {
  const when = earned
  ...
  return `
    <div class="ach-row ${earned ? 'earned' : 'locked'}">
      <span class="ach-icon">${earned ? achievement.icon : '🔒'}</span>
      <div class="ach-what">
        <strong>${esc(achievement.name)}</strong>
        <i>${esc(achievement.blurb)}</i>
      </div>
 ...
 */
export function unlockedPanelHtml(unlocked: readonly Achievement[]): string {
